// brief-v2 — second iteration of the BusinessProfile → DesignBrief generation
// prompt. Copied from brief-v1.ts; targeted updates calibrated against the
// four parse-v1 findings in docs/findings-log.md.
//
// Changes from brief-v1 (see docs/findings-log.md for evidence):
//   1. New "Specific failure modes the corpus exposes" section immediately
//      after "Variety". Names the "sophisticated professional" attractor
//      (sage / cool_premium / editorial_asymmetric / expert_authoritative)
//      and gives explicit guidance on when each is appropriate vs. lazy.
//      Includes a "ten varied businesses" self-check heuristic — if you'd
//      pick the same answer for more than three of ten, you're defaulting.
//   2. New "Domain-vocabulary anchors" subsection inside Vocabulary, after
//      the brandArchetype list. Disambiguates "discerning"/"considered" in
//      fashion (aesthetic discipline, not academic expertise), "data-driven"
//      in non-expert customer contexts (everyman/caregiver, not sage),
//      "premium" in retail (ruler/lover) vs B2B (sage), and "independent"/
//      "boutique" (everyman/creator/lover, not sage).
//   3. Voice-cascade clause appended to the Coherence section. Voice
//      follows brand archetype + personality, not individual service
//      categories — even when a service implies authority (IV drips,
//      legal filings, regulated activities).
//   4. Layout-diversity rule appended to the layoutArchetype enum list.
//      Names editorial_asymmetric as a collapsing default and points at
//      domain-appropriate alternatives (classic_corporate, minimal_typographic,
//      split_screen, grid_showcase, storytelling_scroll, immersive_visual,
//      hero_centric, bento_box).
//
// Sections explicitly NOT touched (per the v1 strengths to preserve):
//   - "How to reason" — the reasoning loop is sound.
//   - "Rationale" — every v1 brief grounded rationale specifically; do not
//     destabilize.
//   - "Output format" / "A specific rule about businessProfile" — landed
//     correctly; verified by post-eval smoke test (pre-injection log read
//     "empty {} (correct)").
//   - OKLCH rules and the three example palettes — palette content was
//     correct in v1; the issue was strategy collapse, addressed in change 1.
//   - User prompt construction.

import { type BusinessProfile } from '@repo/shared-types';

export const PROMPT_VERSION = 'brief-v2';
...
Voice cascades from the brand, not from individual services.
If a business has an informal founder voice, casual brand personality, or playful tone, the voice is warm_friendly, playful_irreverent, confident_direct, or similar — even if some services (medical procedures, legal filings, regulated activities) might individually suggest authority.
Authority is a property of the brand at the highest level, not of individual service categories. A wellness studio offering IV drips and infrared sauna is still a wellness studio. A bakery selling artisanal loaves is still a bakery. Do not let one service category override the voice that the whole brand demands.
If you find yourself writing rationale that says "the voice should be X, but because of [specific service], we pick Y instead," you are breaking the cascade. Pick X.
...
# Specific failure modes the corpus exposes

The "sophisticated professional" attractor.
There is a strong gravitational pull toward four specific choices whenever the business has any signal of expertise, quality, or sophistication: brandArchetype: sage, colorPalette.strategy: cool_premium, layoutArchetype: editorial_asymmetric, and voice: expert_authoritative. This combination is the lazy answer for "this business seems serious."

These choices belong to businesses where credibility-through-restraint is the primary identity:
- B2B consulting firms (consulting is the service, expertise is the product)
- Law firms (the entire value proposition is professional credibility)
- Financial services and wealth management
- B2B enterprise software targeting CTOs and VPs
- Academic and research institutions

These choices are wrong when expertise is one quality among several:
- Fashion brands (even with "discerning" or "considered" copy — those are aesthetic descriptors, not credentialing claims)
- Hospitality, restaurants, bakeries (warmth and place beat expertise)
- Real estate (relationships and trust beat institutional authority)
- Tutoring and education (mentorship beats lecture)
- Retail and ecommerce (customer delight beats expert pronouncement)
- Beauty and wellness (intimacy and sensation beat clinical authority)

Before settling on sage, ask: is this business primarily about evidence and expertise, or is expertise one of several qualities? If the latter, pick a different archetype. A fashion brand with high standards is lover (sensual, intimate) or creator (imaginative, artistic), not sage. A real estate buyer's agent who is meticulous is everyman (approachable expert) or caregiver (advocate), not sage. A tutoring service is usually caregiver (helping students grow), not sage.

A heuristic for self-checking. Imagine the briefs you would produce for a corpus of ten varied businesses (a gym, a clinic, a fashion brand, a law firm, a restaurant, a SaaS product, a real-estate agency, a creative studio, a tutoring service, a wellness studio). How many of those briefs would land on sage archetype? On cool_premium palette? On editorial_asymmetric layout? On expert_authoritative voice? If your honest answer is "more than three of the ten," you are using these choices as defaults. Pick again for any business that doesn't primarily lead with restraint and authority.
...
Domain-vocabulary anchors. Specific patterns the model frequently misreads:
- "Discerning," "considered," "edited," "curated," "thoughtful" in fashion or retail context describe aesthetic discipline, not academic expertise. Read these as lover, creator, or ruler signals, not sage.
- "Data-driven," "evidence-based," "research-backed" in a service context where the customer is non-expert (real estate, financial advising, healthcare for non-specialists) often map to everyman (translating expertise for the customer) or caregiver (advocating for the customer), not sage (where expertise is the product itself).
- "Premium," "luxury," "elevated" in retail or hospitality map to ruler or lover. In B2B contexts they may map to sage. Context is the discriminator.
- "Independent," "small," "boutique," "founder-run" usually signal everyman, creator, or lover — not sage. Independence as a value is community-coded, not authority-coded.
...
editorial_asymmetric is not the sophisticated default. It belongs to editorial brands, fashion houses, magazines, and design agencies whose visual identity is the magazine-page aesthetic. It is wrong for most consultancies, law firms, real estate agencies, retail, and hospitality.
Common alternatives by domain:
- Consultancies and law firms: classic_corporate (structure signals trust) or minimal_typographic (restraint signals confidence)
- Real estate: split_screen (property image + content) or grid_showcase (listings, neighborhoods)
- Restaurants, bakeries, hospitality: storytelling_scroll (narrative across the page) or immersive_visual (full-bleed photography)
- Fitness, gyms: hero_centric (one bold statement) or grid_showcase (programs, classes)
- Modern tech, SaaS: bento_box (mixed-size feature tiles) or hero_centric
Pick the layout that matches the business's operating model and content type, not its desired sophistication level.
